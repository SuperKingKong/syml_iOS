...
static const NSInteger kContentIsTooMuchErrorCode = 999999;
...
- (void)connection:(NSURLConnection*)connection didReceiveResponse:(NSHTTPURLResponse*)response {
  ...
  NSInteger contentLength = [[headers objectForKey:@"Content-Length"] floatValue];
    ...
    //TTDPRINT(@"response header=%@",headers);
  ...
  if (contentLength > _queue.maxContentLength && _queue.maxContentLength) 
  {
    ...
    NSError* error = [NSError errorWithDomain:NSURLErrorDomain code:kContentIsTooMuchErrorCode userInfo:nil];
    [_queue loader:self didFailLoadWithError:error];
...
- (void)connection:(NSURLConnection*)connection didReceiveData:(NSData*)data 
{
    ...
    CGFloat nowLength = [[NSNumber numberWithInteger:_responseData.length] floatValue];
