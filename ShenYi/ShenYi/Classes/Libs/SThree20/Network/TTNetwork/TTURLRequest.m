...
- (id)init {
  if (self = [super init]) {
    ...
    _charsetForMultipart = NSUTF8StringEncoding;
      self.requestCache = [TTURLCache sharedDocsCache];
  }
  return self;
}
...
- (NSString*)generateCacheKey {
  if ([_httpMethod isEqualToString:@"POST"]
    ...
    for (id key; key = [e nextObject]; ) {
	  ...
	  return [SThree20Extral md5Hash:joined];
  } else {
	  return [SThree20Extral md5Hash:self.urlPath];
  }
}
